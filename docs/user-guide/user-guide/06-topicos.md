...
## 6.3 Topicos de eventos de saúde

Os quatro topicos abaixo sao publicados pelo monitor de saúde proativo quando `events.enabled: true` (veja [7.7 Eventos de saúde proativos](07-configuracao.md#77-eventos-de-saúde-proativos)).

Eles **nao sao editaveis pela pagina Topics** — para customizar esses padroes, edite diretamente o arquivo `config.yaml` na secao `mqtt.topics` ou use a API (`PUT /api/config`).

| Campo | Direcao | Padrao | Descricao |
|-------|---------|--------|-----------|
| `health_alerts` | Publish | `smartreader/%s/healthAlerts` | Publicado quando temperatura cruza limiares de warning ou critical. |
| `network_status` | Publish | `smartreader/%s/networkStatus` | Publicado quando WiFi ou MQTT broker conectam/desconectam. |
| `antenna_status` | Publish | `smartreader/%s/antennaStatus` | Publicado quando antena e detectada como com defeito ou se recupera. |
| `inventory_status` | Publish | `smartreader/%s/inventoryStatus` | Publicado quando inventario inicia ou para. |

> **Nota:** Para habilitar esses eventos, configure `events.enabled: true` na pagina Config (secao [7.7](07-configuracao.md#77-eventos-de-saúde-proativos)).
> Mais detalhes sobre os formatos dos eventos em [10. Monitoramento de Saúde](10-monitoramento-saude.md).

---

## 6.4 Pre-visualizacao ao vivo
